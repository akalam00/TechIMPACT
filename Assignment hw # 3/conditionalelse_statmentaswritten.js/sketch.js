...
Created by Professor D
 by Abul Kalam and added comments as needed. 
*/
...
var recX = 100;// variable recX is set to a value.
...
var canvBG = 225;// variable canvBG is set  to a value.
...
function setup() { // setup ()function is the start of the code and gives us the access to the size of the window.
	createCanvas(400, 400); // the size of the canvas is 400 by 400.
}
...
function draw() { // function draw() makes the code compile as the order follows. 
	...
	ellipse(circX + 50, circY - 75, circSize, circSize); // this ellipse function is going to tell us its value of the coordinates and run the program as naccesary.
	...
	if (circX < width) { // the if statement is telling us to check the circle if the variable is less than the width and try to check if its true or false
		...
		circSize += 0.5; // This circle size will increase by 0.5 through each loop and check if the statement is true and if not it will continue
		...
		if (circX > width) { // this time we are checking if circX is greater than the width
			...
			circSize *= -1; // This circle size will decrease by number multiplied by -1 through each loop and compile until its finish
