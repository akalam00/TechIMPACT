/* Created by Abul Kalam two function project
...
function draw() {
 ...
 bounce ();
	 ...
	 for (var c = 0; c <= width; c += 20) { // for every i add 60. Spaces rectangles out. 
		for (var d = 0; d <= height; d += 20) { // for every j add 60. Spaces rectangles out.
			fill(mouseX, random(255), mouseY); // fill based on a random value and the mouse position
			triangle (c, d, 25, 25,68 ,68 ); // draw at i, j for each rectangle's x and y position. 
		}
	}
}
